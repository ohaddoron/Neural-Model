function EPSC = solve_u_xV1 (spikeMat,tmax,dt,A,U,tauD,tauF)

% In this version the equations used to solve were taken from the paper Differential signaling via the same axon of neocortical
% pyramidal neurons
...
for i = 1 : size(spikeMat,1)
    ...
    for n = 1 : Nt-1
        ...
        cur_u(n+1) = cur_u(n) * exp(-DeltaT/tauF) + U*(1-cur_u(n)*exp(-DeltaT/tauF));
        cur_x(n+1) = cur_x(n) * (1 - cur_u(n+1)) * exp(-DeltaT/tauD) + 1 - exp(-DeltaT/tauD);
    end
    u = [u; cur_u];
    x = [x; cur_x];
